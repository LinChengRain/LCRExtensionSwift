...
extension Date {
    //MARK: - 获取日期各种值
    //MARK: 年
    public func lcr_year() ->Int {
        let calendar = NSCalendar.current
        let com = calendar.dateComponents([.year,.month,.day], from: self)
        return com.year!
    }
    //MARK: 月
    public func lcr_month() ->Int {
        let calendar = NSCalendar.current
        let com = calendar.dateComponents([.year,.month,.day], from: self)
        return com.month!
    ...
    //MARK: 当月天数
    public func lcr_countOfDaysInMonth() ->Int {
    ...
    //MARK: 当月第一天是星期几
    public func lcr_firstWeekDay() ->Int {
    ...
    //是否是这个月
    public func lcr_isThisMonth()->Bool {
        let calendar = NSCalendar.current
        let com = calendar.dateComponents([.year,.month,.day], from: self)
        let comNow = calendar.dateComponents([.year,.month,.day], from: Date())
    ...
    public func lcr_getNewDate(_ date: Date?, addDays days: TimeInterval) -> Date? {
        // days 为正数时，表示几天之后的日期；负数表示几天之前的日期
        return addingTimeInterval(TimeInterval(60 * 60 * 24 * days))
    }
    
    //MARK: 日
    public func lcr_currentDay() ->Int {
        let calendar = NSCalendar.current
    ...
    // 计算两个日期之间的时间间隔多少天
    public static func lcr_dateInterval(startTime:String,endTime:String) -> Int{
